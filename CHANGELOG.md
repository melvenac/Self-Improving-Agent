...
### Fixed
- **A renamed project directory no longer silently strands its own memory.** `project_dir` is an identity key derived from a path, so renaming `Tarrant County Makerspace` to `Tarrant-County-Makerspace` — spaces are miserable to `cd` into — left **16 knowledge entries and 15 vault notes keyed to a directory that does not exist**. A project-scoped `ob_recall` from the real repo returned roughly 60% of that project's memory and looked exactly like a complete result while doing it. New `open-brain relocate --from <dir> --to <dir>` folds the stored history forward: `knowledge_index.project_dir`, `sessions.project_dir`, and the vault notes themselves, in one transaction. Dry-run by default per the `dream` convention; `--apply` is the only path to a write. Name collisions are reported and **never** overwritten — a same-named note in both folders is two notes, not one to replace — and a note filed outside the project folder keeps its location rather than being re-parented by a rename that never implied it.
- **`canonicalizeProjectDir` deliberately still does not fix this, and should not.** Folding spaces to hyphens would corrupt every project whose directory legitimately contains one. A rename is information the filesystem cannot supply; someone has to say "these two are the same project."

### Added
- **`project-dirs` sync check — the detector the rename needed and did not have.** Warns when any stored `project_dir` no longer resolves on disk, naming each one with its entry count and the `relocate` command to fix it. Asks the filesystem rather than guessing a normalization rule, so it catches any rename, move or deletion. **Warn, never issue:** a directory can be legitimately absent (archived project, another machine, work that was never a repo), and blocking the commit gate over old history is the wrong trade. It immediately found a second orphan nobody had noticed — `c:/users/melve/nanopc-t4`, 4 entries and 2 sessions, with no directory anywhere on disk.
- **`open-brain topics [--min=<n>] [--apply]` — the connective layer `vault-writer` never emitted.** Generates one `Topics/<tag>.md` per subject tag, linking every entry that carries it. The v2 graph was one hub (`SKILL-CANDIDATES.md`, 1,354 links) ringed by isolated nodes, because nothing writes wikilinks and v1's apparent richness came from **21 hand-written Maps of Content last modified in March** whose links point at v1's old date-and-hash filenames — uncarryable, and frozen since. Tags are the right signal: 460 of 464 entries carry them against 170 with a project. Type-shaped tags (`gotcha`, `pattern`, `research`…) are excluded — a 177-entry `gotcha` topic is the mega-cluster shape from entry 303, a group too large for anyone to read. Links use the basename when unique and the vault-relative path when it repeats, because an ambiguous `[[name]]` silently resolves to the wrong note. Idempotent, and **only files carrying `generated: open-brain-topics` are ever rewritten or deleted**, so a hand-written note in `Topics/` is left alone.

### Notes
- **Applied to the live vault:** TCM merged to 40 entries under one key, 15 notes moved, the emptied split folder removed. Topics generated at `--min=5` → **128 topic notes**. Vault link coverage went from 27 files / 1,406 links to **155 files / 2,975 links**, with **444 of 448 experience notes** now reachable from at least one topic. Re-runnable at a higher threshold (`--min=10` ≈ 60 notes); topics falling below threshold are removed on the next run.
- **This changes no retrieval behaviour.** `ob_recall` reads SQLite FTS5 and skill-scan clusters by tag; nothing in the system follows an Obsidian wikilink. The graph is a human browsing affordance, and keeping that straight matters so a sparse graph is never mistaken for broken memory. The `relocate` half **does** change retrieval — that one was a real bug.
- **717 tests / 52 files** (+18) at v0.23.0, **+23 more here**. `nanopc-t4` left alone deliberately: no directory exists anywhere, so it is a deletion rather than a rename, and only Aaron can say whether that history should be re-homed or retired.

## [v0.23.0] - 2026-09-01

### Fixed
- **Apoptosis archives instead of destroying** (calibration analysis by Ledger, a read-only peer session; built by Forge). The auto-prune path already archived the vault note — "apoptosis fires with no human in the loop, so destroying a readable note automatically is the wrong default" — and then hard-`DELETE`d the row anyway, which was the more destructive half of the same decision. Neither `feedback_log` nor `recall_log` declares a foreign key (`foreign_key_list` returns `[]` on both), so the ratings that *justified* the prune survived as rows pointing at an id that no longer resolved: the evidence became unreadable at the moment it was acted on, and an entry pruned for being bad became indistinguishable from one never rated at all. Now `archiveKnowledgeEntry` sets `archived_into` and applies the final rating in the same statement, so the archived row records the verdict that retired it rather than the state just before. Reversible by clearing one column; `ob_forget` remains the way to remove an entry for good.
- **`archived_into` is written for the first time.** The column was read in ~15 places and written nowhere — all 464 live entries were `NULL` — so twelve production `archived_into IS NULL` filters were no-ops guarding a state nothing could enter. Apoptosis has no successor entry to point at, so `ARCHIVED_NO_SUCCESSOR = 0` means "retired, no successor": `knowledge_index.id` is AUTOINCREMENT from 1, so zero cannot collide with a real merge target, and every existing `IS NULL` filter keeps working untouched.
- **The pruner and `ob_stats` no longer disagree about who is eligible.** `db-v2.ts` gated on `helpful + harmful + neutral >= 5` against a hardcoded `0.3` while `evaluateLifecycle` gated on `helpful + harmful` against `LIFECYCLE_CONFIG` — so the health stat and the mechanism it claims to describe counted different populations (56 entries vs 23 in the 2026-09-01 snapshot, a 2.4x gap). Both surfaces returned zero, which is why it went unseen, and **both zeros were the structurally-unreachable kind, not the healthy kind** — the third instance of that shape in this subsystem. Gate and threshold now come from new `apoptosisGateExpr` / `lowSuccessExpr` in `lifecycle.ts`, which `apoptosisFlaggedExpr` and the staleness stats both build from.

### Notes
- **The four-part lifecycle bundle shipped as one part, deliberately.** Ledger's ruling: **no defensible apoptosis threshold exists**, so the `success_rate` denominator fix and threshold re-tune are *not* shipped. `session-end/index-v2.ts:106` falls back to `tags.some(tag => summaryLower.includes(tag))` when no explicit rating is supplied — a topic-mention detector, structurally unable to emit `harmful`. So `success_rate` is a citation rate wearing a quality label, `harmful` has fired twice in the corpus's life, and at `t=0.3` the gate would auto-delete 22 entries that all hold spotless records — including one at 77 recalls, and entry 351, `high-base-rate-signal-cannot-discriminate`, which describes this exact failure mode. Threshold work is blocked on making the default rating path capable of expressing harm; **measure that compliance gap before fixing it**, per the recall-path precedent. Part 3 (compile-before-prune) is unshippable: no objective gating signal exists anywhere in schema or source.
- **Correction to prior analysis:** the widely-quoted `0.311` per-entry mean was already neutral-inclusive, not the pre-fix figure. True old-denominator mean is **0.9865** — with `harmful ≈ 0`, `helpful/(helpful+harmful)` degenerates to 1.0. Separately, `knowledge_index` counters hold 1,157 events against 347 `feedback_log` rows because 323 of 464 entries predate both logs, so every `feedback_log`-derived census described ~30% of what the pruner actually reads. Full working: `~/Obsidian Vault v2/Research/lifecycle-calibration-2026-09-01.md`.
- **699 tests / 50 files** (+11). `db.ts` confirmed dead — zero importers, and its `knowledge` table does not exist in the live database; its stale copies of the formula and threshold were left alone rather than fixed, and it is a deletion candidate.
