#!/usr/bin/env node
/**
 * Knowledge DB Dashboard — Read-only web viewer for knowledge-v2.db
 * Run: node open-brain/scripts/dashboard.mjs
 * Opens: http://localhost:3456
 *
 * v2 note: this viewer was written against the retired v1 schema
 * (knowledge / summaries / tags / chunks_fts). The v2 store uses
 * knowledge_index and has no summaries or tags tables. Rather than
 * mutate knowledge-v2.db, the shim below creates TEMP views and a TEMP
 * FTS table scoped to this connection only — nothing is persisted.
 */
...
// --- v1 -> v2 compatibility shim (TEMP only; main DB untouched) ---
...
try {
  if (!hasTable("knowledge") && hasTable("knowledge_index")) {
    // knowledge_index carries every column the viewer reads, plus extras.
    db.exec(`CREATE TEMP VIEW knowledge AS
      SELECT id, key, content, tags, source, maturity, project_dir,
  ...
  // v2 dropped these tables entirely — expose empty, correctly-shaped views
  // so the viewer's queries still parse and return zero rows.
  if (!hasTable("summaries")) {
    db.exec(`CREATE TEMP VIEW summaries AS
      SELECT NULL AS id, NULL AS session_id, NULL AS summary, NULL AS model,
             NULL AS created_at, NULL AS project_dir
      WHERE 0`);
  }
  if (!hasTable("tags")) {
    db.exec(`CREATE TEMP VIEW tags AS
      SELECT NULL AS chunk_id, NULL AS tag WHERE 0`);
  }
  if (!hasTable("summaries_fts")) {
    db.exec(`CREATE VIRTUAL TABLE temp.summaries_fts USING fts5(summary)`);
  }

  // v2 has no chunks_fts. Build it in temp space from the live chunks table
  // so chunk search works without writing to knowledge-v2.db.
  if (!hasTable("chunks_fts") && hasTable("chunks")) {
    db.exec(`CREATE VIRTUAL TABLE temp.chunks_fts USING fts5(category, content)`);
    const rows = db.prepare("SELECT id, category, content FROM chunks").all();
    const ins = db.prepare(
      "INSERT INTO temp.chunks_fts(rowid, category, content) VALUES (?, ?, ?)"
    );
    for (const r of rows) ins.run(r.id, r.category, r.content);
    console.log(`  shim: indexed ${rows.length} chunks into temp FTS`);
  }
} catch (err) {
  console.error(`Compatibility shim failed: ${err.message}`);
  process.exit(1);
}
