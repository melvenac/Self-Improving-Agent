...
- **Open questions:** anything unresolved that needs {{USER_NAME}}'s input
...
If knowledge was recalled during `/start`, self-evaluate each entry — don't ask {{USER_NAME}}.
...
5. Report ratings to {{USER_NAME}} (they can override if needed)
...
**Why self-evaluate:** {{USER_NAME}} can't see whether recalled knowledge helped the agent's internal reasoning. The agent that consumed it is the only one who knows.
...
- If {{USER_NAME}} says "don't store that," respect it immediately.
