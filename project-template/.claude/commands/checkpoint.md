...
1. Run `ob_recall` with query: `"[CHECKPOINT]"` scoped to the current project, limited to the last 1 session (`sessions: 1`), and `trigger: "checkpoint"` — this recall is checkpoint machinery, recorded distinctly so analyses can pool it with or separate it from session-start injection at query time
